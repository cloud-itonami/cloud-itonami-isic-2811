(ns turbine.operation
  "OperationActor -- one engine-turbine-manufacturing operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Turbine Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Turbine plant Manufacturing
  ...
  One graph run = one engine-turbine-manufacturing operation (intake ->
  ...
  decision to a human operator (the turbine manufacturing engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/dispatch-unit`/`:actuation/issue-
  type-evidence` ALWAYS reach this node when the governor is
  clean -- see `turbine.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [turbine.turbineadvisor :as turbineadvisor]
            [turbine.governor :as governor]
...
(defn build
  ...
  `turbine.store/Store`).
  opts:
    :advisor      -- an `turbine.turbineadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (turbineadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Turbine Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (turbineadvisor/-advise advisor store request)]
            {:proposal p :audit [(turbineadvisor/trace request p)]})))

      ;; Turbine Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
